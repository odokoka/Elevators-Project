...
void Building::spawnPerson(Person newPerson){
    int current_floor = newPerson.getCurrentFloor();
    int request = newPerson.getTargetFloor() - current_floor;
    floors[current_floor].addPerson(newPerson, request);
}

void Building::update(Move move){
    int newListOfPeopleToRemove[MAX_PEOPLE_PER_FLOOR] = {};
    if (move.isPassMove()) {
        return;
    } else if (!move.isSaveMove() && !move.isQuitMove()) {
        elevators[move.getElevatorId()].serviceRequest(move.getTargetFloor());
    }
    if (move.isPickupMove()) {
        move.copyListOfPeopleToPickup(newListOfPeopleToRemove);
        floors[elevators[move.getElevatorId()].getCurrentFloor()].removePeople(newListOfPeopleToRemove, move.getNumPeopleToPickup());
    }
    return;
}
...
int Building::tick(Move move){
    ...
    for (int j = 0; j < NUM_FLOORS; ++j) {
        numExplodedPeople += floors[j].tick(time);
    }
