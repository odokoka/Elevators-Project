...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    ...
    //Big foor loop to iterate through all of pickup list
    for (int i = 0; i < pickupList.length(); ++i) {
        bool directioni;
        //Checks for negatives
        int stringNum = pickupList[i] - '0';
        if (stringNum < 0) {
            return false;
        }
        //Checks if highest number is greater than num people on floor
        if (stringNum >= building.getFloorByFloorNum(pickupFloorNum).getNumPeople()) {
            return false;
        }
        
        //checks for direction with i
        if (building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(stringNum).getTargetFloor() - building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(stringNum).getCurrentFloor() > 0) {
            directioni = true;
        } else {
            directioni = false;
        }
        ...
        for(int j = 0; j < pickupList.length(); ++j){
            bool directionj;
            int stringNum2 = pickupList[j] - '0';
            if((stringNum == stringNum2) && (i != j)) {
                return false;
            }
            //Checks for direction with j
            if (building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(stringNum2).getTargetFloor() - building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(stringNum2).getCurrentFloor() > 0) {
                directionj = true;
            } else {
                directionj = false;
            }
